...
### 2026-07-14 (latest)
- **Summary readiness gate matches the real threshold — no more TOC dead-end (#218, P2.17, bug/ux)**: frontend-only, PR #294. The summary page enforced `MIN_SUMMARY_LENGTH = 30` **characters** while the counter said "Minimum: 30 characters", the guideline two lines below said "Minimum 30 **words** recommended", and the NOT_READY screen claimed "at least 500-1000 words" — four numbers, one enforced. **Verified before building**: the only real gate is `word_count >= 30 and char_count >= 150`, **identical on both backend paths** (`books.py` deterministic fallback + `ai_service.py` AI path — the AI does *not* decide readiness), and the wizard gates on `meets_minimum_requirements` alone, ignoring `is_ready_for_toc`. The page's `30` and the backend's `30` were the same digits in **different units** — a coincidence inviting a wrong fix. New `lib/constants/summary-readiness.ts` mirrors the gate as a documented SSOT (the `auto-save.ts` pattern); page delegates to it, counter shows words **and** characters against both real minimums, every fictional number (incl. `bookClient`'s "100+ words" doc comment) replaced by the enforced one. Took a **hard gate at the real threshold** over the AC's soft-warning alternative (a warning still lets the user walk into the dead-end). **The gate is only as good as its arithmetic — two JS/Python counting divergences each recreate the bug, both verified against the real runtimes, not reasoned about**: (1) *whitespace* — JS `/\s/` ≠ Python `str.isspace()`: `\x85`/`\x1c-\x1f` (Python splits, JS doesn't → falsely rejects a valid summary) and **`﻿` BOM** (JS splits, Python doesn't → accepts one the backend rejects; BOMs are common in pasted text), so the module spells out Python's exact set; (2) *characters* — **codex pre-PR P2**: JS counts UTF-16 code units, Python counts code points (`len('👍'*100) == 100` vs `.length === 200`), so a mostly-emoji summary cleared the 150 gate then failed the backend's 100 → `Array.from().length`. **No data loss**: the debounced auto-save persists independently of validation, so a user under the stricter gate keeps their draft (pinned). Reviews: opencode (GLM) hung 10 min zero-output (the documented condition) → **codex fallback pre-PR (the emoji P2, adopted + pinned) + post-PR fresh session clean**; **GLM CI review ×2 "no defects found"**, independently re-verifying the gate on both backend paths. Tests: **33 pins across 3 suites** — first tests this page has ever had — all RED-verified at birth; mutations: old char-only gate → 3 page pins fail, "500-1000 words" restored → 2 fail, `/\s+/` → 6 parity pins fail, `text.length` → 2 emoji pins fail. Demo (`docs/demos/2026-07-14-issue-218-summary-readiness-gate.md`): real backend + genuine better-auth signup, branch :3000 vs pristine main worktree :3001, same session/book — **main tells the user "30 characters / Minimum: 30 characters" (met), enables submit, then bounces them to "Summary Needs More Detail — Word Count 5, Meets Requirements: No, aim for 500-1000 words"**; branch reports "5 / 30 words" + "30 / 150 characters" (byte-for-byte the backend's own counts) and blocks first; emoji case shown live (backend 106 / main's counter 152 + enabled / branch 106 + blocked while raw `value.length` is still 152). Control: main with the same ready summary hits the identical stale-key AI 401, so the wizard's "Something Went Wrong" is environmental, not the diff. Frontend **121 suites, 2181 passed / 5 skipped**. Follow-up **#295 [P3.15]**: `SummaryInput.tsx` carries the identical stale copy but has zero importers (#200/#209 dead-code class). **Env note**: the shared mongod on :27017 was wedged and :27018 belongs to another workload — neither touched; the demo ran its own loopback mongod on :27099 (backend + better-auth `DATABASE_URL` both repointed — better-auth has its *own* Mongo connection, defaulting to 27017).
  - **Status**: ✅ Complete
- **2FA backup codes: regenerate action, forced save acknowledgment, sign-in recovery path (#217, P2.16, security/ux)**: frontend-only, PR #293. Backup codes were shown exactly once during enable→verify with no way to view or regenerate them afterwards, no "these won't be shown again" warning, and `verify-2fa` offered no recovery path — a user losing both authenticator and codes had no in-product way forward. Three fixes: (1) **Regenerate backup codes** in the enabled state, password-gated via better-auth's native `authClient.twoFactor.generateBackupCodes({ password })` (verified against installed 1.4.9 types — plugin-inferred via `twoFactorClient`, no cast changes; the existing cast layers on `PasswordResetMethods` and doesn't strip plugin types); new `codes` step shows the fresh set with an explicit "your old backup codes no longer work" warning + Copy. (2) **Forced acknowledgment** on both the enable flow's Verify & Enable and the regenerate flow's Done — the regenerate gate was a codex pre-PR Major (old codes are *already* invalidated at that point, so unacknowledged dismissal recreated the lockout risk). `codesAcknowledged` resets to `false` on entry to both steps, so no stale ack leaks between flows. (3) `verify-2fa` gains a native `<details>` disclosure — "Lost access to your authenticator and backup codes?" — with recovery instructions via support@autoauthor.com (the address already published on /dashboard/help). **No "view current codes"**: better-auth stores codes hashed/encrypted server-side; regenerate is the client-exposed, password-gated path. Reviews: opencode (GLM) occupied by a foreign-cwd delegation (the documented hang condition) → **codex fallback** pre-PR (the regenerate Major) + post-PR fresh session (P2: the enable ack lived only on the disabled button, but Enter in the code input submits the form directly and would bypass it → `handleVerifySubmit` now guards on `codesAcknowledged` as the source of truth, `fireEvent.submit`-pinned). **GLM CI review Major adopted** (09be478): `handlePasswordSubmit` had no re-entry guard — a double-Enter on the regenerate password step fires `generateBackupCodes` twice, and each call *replaces* the server-side code set, so the saved codes desync from the valid ones (lockout); `if (isSubmitting) return;` at handler entry mirrors `verify-2fa`'s `submitCode` idiom (the #205 single-flight class) and covers all three branches. GLM's `handleVerifySubmit` double-fire candidate **rebutted by GLM itself** — `verifyTotp` is idempotent, worst case a spurious toast; final GLM pass "no new defects found". Tests: 6 pins in `SecuritySettings.test.tsx` + `VerifyTwoFactorPage.test.tsx`, **RED-verified at birth** (5/5 fail pre-implementation; the regenerate-Done gate, the Enter-bypass pin, and the held-promise double-submit pin each RED pre-fix); mutations — stripping the ack gate fails exactly the ack pin, short-circuiting the codes step fails exactly the regenerate pin, stripping the single-flight guard fails exactly that pin (15/16). Demo (`docs/demos/2026-07-14-issue-217-2fa-backup-codes.md`): branch :3000 vs pristine main worktree :3001, shared real backend + local Mongo, genuine better-auth signups and a real RFC-6238 TOTP (stdlib Python) — **outcome evidence that regeneration truly invalidates server-side**: same user, same sign-in, an OLD captured code is rejected on verify-2fa while a NEW one lands on /dashboard; main shows an armed Verify & Enable with no warning/checkbox, `Disable 2FA` as the only enabled-state action, and `{details:0, mailto:0, lostAccessText:false}` on verify-2fa. Frontend **118 suites, 2148 passed / 5 skipped**. NB the demo's live signups/code consumption are one-way, so `showboat verify` intentionally diffs on those blocks (#203 precedent). Process note: the pre-commit secrets hook false-positives on `const passwordInput = screen.getByLabelText('Confirm your password')` (the `password` + 8+-char quoted string pattern) — renamed the local, never `--no-verify` (#198 lesson).
  - **Status**: ✅ Complete
- **Account deletion gets the type-to-confirm safeguard single-book deletion already had (#216, P2.15, ux/high)**: frontend-only, PR #292. The irreversible account delete (cascades to every book, #179) was a plain Cancel/Delete dialog while `DeleteBookModal` required typing the book title — the most destructive action had the weakest guard. The profile-page dialog now requires typing the **account email** before "Delete account permanently" enables, mirroring DeleteBookModal semantics: case-sensitive exact match, `role="alert"` mismatch hint with `aria-invalid`/`aria-describedby`, Enter-submits form wrapper (sibling of the profile form — dialog portals out), input reset on every open/close, handler re-entry guard, and mid-delete dismissal blocked (`onPointerDownOutside`/`onEscapeKeyDown` + the controlled open-change guard — deliberately double-guarded; the Escape pin only goes RED when both are stripped). **Phrase source**: `session.user.email ?? hydrated profile email` — the fallback is what makes route-mocked E2E (no real session) workable, pinned by a dedicated unit test; both absent → fail-closed disabled button (unreachable in production, GLM M3 accepted). No plan comment on the issue — plan self-authored. Tests: new `ProfilePageDelete.test.tsx` (8 tests, real RHF + real Radix Dialog + real `useProfileApi` over mocked `authFetch` — the `ProfilePageSave` idiom; the legacy suite mocks those wholesale and can't express disabled semantics), **RED-verified at birth (5/5 fail pre-implementation) + 3 mutations**: `deleteConfirmed=true` fails exactly the 3 guard tests; stripping the `deleting` re-entry guards fails exactly the double-submit pin; stripping both close guards fails exactly the Escape pin. E2E: delete-guard test in `profile-editing.spec.ts` (chromium 4/4). Reviews: opencode (GLM) pre-PR **"clean to merge"** (M1 dismissal-guard parity, M2 session-mock re-seed, M4 E2E fallback comment adopted) + post-PR fresh session **"clean to merge... no bypass vectors"** (m1 trigger→open-change handler + 3 test-strength pins adopted; m5 Enter-keydown rebutted — jsdom implements no implicit form submission, `fireEvent.submit` is the idiom); GLM CI review ×2 **"no defects found"**. Demo (`docs/demos/2026-07-14-issue-216-account-delete-type-to-confirm.md`): branch :3000 vs pristine main worktree :3001, shared real backend + local Mongo + two genuine better-auth signups — **main deletes the account in one click with zero friction** (Mongo `is_active` true→false shown) vs branch disabled→case-mismatch alert→armed only on the exact email→confirmed deletion lands the same Mongo state; `showboat verify` diffs only on post-session browser-eval blocks + one-way Mongo state (#203 precedent). Frontend CI **118 suites, 2142 passed / 5 skipped**. Process note: a mutation-check `git checkout` restore wiped uncommitted work again (the #201 lesson) — re-applied; rule promoted to persistent memory: commit first, mutate second.
  - **Status**: ✅ Complete

### 2026-07-14
- **BookCard + EmptyBookState respond to the theme — dashboard no longer half-dark in light mode (#206, P2.14, bug/ui)**: frontend-only, PR #290. Both first-run dashboard surfaces hardcoded dark-theme literals (`bg-gray-800/border-gray-700/text-gray-*`, dark-tinted `indigo-*`); switching to Light rendered a dark book grid under light chrome. Migrated to the shared token vocabulary (`bg-card`/`border-border`/`text-card-foreground`/`text-muted-foreground`/`bg-muted`, `bg-primary/10 border-primary/50` callout, `bg-secondary` buttons with `hover:bg-primary` / `hover:bg-destructive` per the ChapterTab precedent). **Plan adaptation (verified)**: no global `* { @apply border-border }` rule exists (the globals.css base rule reads a v4-only `@theme inline` var this v3 project ignores) — explicit `border-border` required. **Pre-PR codex Major (plan was wrong)**: the CodeRabbit plan's `text-primary` for brand text is a *dark-mode* contrast regression — `primary` is FIXED RGB indigo-600 here (~2.3:1 on dark card) — so brand text uses the repo's two-tone precedent (`text-indigo-600 dark:text-indigo-400/300`, as in PasswordRequirements/sign-in); fills/borders keep `primary`. Tests: 10 token pins (RED + mutation verified) + a per-element sweep banning `gray-*` outright and allowing `indigo-*` only in a `dark:` two-tone pair (post-PR codex Minor adopted); new `EmptyBookState.test.tsx`. Reviews: opencode (GLM) hung 10 min both rounds → **codex fallback pre-PR (the Major) + post-PR fresh session (no Critical/Major)**; GLM CI review ×2 **"no defects found"**. Demo (`docs/demos/2026-07-13-issue-206-theme-tokens.md`, **showboat verify exit 0**): branch :3000 vs pristine main worktree :3001, shared real backend + Mongo + genuine better-auth signup — main's dark empty-panel/card floating on the light dashboard captured live; branch correct in both themes. **Demo-found, fixed in-PR**: the dashboard page title "My Books" was itself `text-gray-100` (invisible in light) → `text-foreground`. **Demo-found, pre-existing, filed #291 [P2.23]**: `GET /books/` returns no `chapters`/`progress`, so cards render "undefined chapters"/a bare "%" and the `chapters === 0` "New" callout branch is unreachable from the live dashboard. Frontend **117 suites, 2144 passed / 5 skipped**.
  - **Status**: ✅ Complete

### 2026-07-13
- **Orphaned /dashboard/new-book route removed — required Genre/Target-Audience no longer silently dropped (#205, P2.13, bug/data-loss)**: frontend-only, PR #289. The unreachable page collected REQUIRED genre/target_audience but `handleSubmit` sent only `{title, description}` — demo pins it live: a fully-filled submit on main persists `genre: null, target_audience: null` with a success redirect; branch 404s the route. Took the plan's **delete** branch with two drift corrections the 2026-07-04 CodeRabbit plan predated: (1) **#201 (merged same day) had pointed the CI-run E2E** (`error-recovery-flow`, `complete-authoring-journey` step 1) at the orphaned page — both retargeted to drive the BookCreationWizard modal from the dashboard (Radix selects; `.first()` — the empty-state renders a second "Create New Book"); (2) the wizard's error path was **weaker** than the deleted page's (generic `toast.error`, no classification, no Retry) so the #201 pins were unsatisfiable against the canonical flow — the page's `classifyError` + `showErrorNotification({onRetry})` handling is now ported into the wizard (dialog stays open, form intact). **App-wide bug found via the retargeted retry test**: Radix modal dialogs set `pointer-events: none` on `<body>`, which the sonner toaster inherits — **every toast action button (Retry, the #247 Upgrade CTA) was unclickable while any dialog was open** ("dialog-overlay intercepts pointer events"); the shared `ui/sonner` Toaster now forces `pointerEvents: 'auto'` (sonner's `style` prop lands on `ol[data-sonner-toaster]`, verified in source). Taxonomy: new `src/lib/constants/book-metadata.ts` (`GENRE_OPTIONS` 11-item wizard list canonical, `TARGET_AUDIENCE_OPTIONS`) consumed by both `BookCreationWizard` and `BookMetadataForm` — the Edit form gains its missing Historical/Biography/Self-Help/Business options; schema stays free-text (backend contract unchanged). GLM CI review minor adopted: **single-flight ref guard on `onSubmit`** — the toast Retry bypasses the disabled submit button, so a double-click POSTed a duplicate book (RED-verified: 3 POSTs without the guard, exactly 2 with). Page-objects/deployment specs repointed at the modal (genre/audience helpers resolve labels from the shared constants; `verifyFormNotTransparent` checks the DialogContent — codex pre-PR P2; unused `clickNewBook` deleted; 04 perf budget measures modal open). All new behavior pins RED-verified (wizard classified-error tests, taxonomy pin, single-flight, and the E2E retry test failing on overlay interception pre-fix). Reviews: opencode (GLM) hung 10 min → **codex fallback pre-PR (1 P2) + post-PR fresh session clean**; GLM CI review 1 minor (the guard). Demo (`docs/demos/2026-07-13-issue-205-remove-orphaned-new-book.md`): branch :3000 vs pristine main worktree :3001, shared real backend + local Mongo — data-loss differential with post-create API docs, canonical-vs-divergent genre lists in the live Edit form, outage → classified TRANSIENT toast + Retry over the open dialog, E2E retry-recovers run in-demo; `showboat verify` diffs only on the 3 live-browser snapshot blocks (#203 precedent). Frontend **116 suites, 2123 passed / 5 skipped** (NewBookPage.test deleted; +2 wizard pins, +1 taxonomy, +1 single-flight); chromium E2E 74 passed / 7 skipped; net −331 lines. NB TRANSIENT toasts auto-dismiss in 5s — too fast to click Retry across a ~6s backend restart in a live demo; the E2E (route-abort injection) is the reproducible proof.
  - **Status**: ✅ Complete

- **Profile-page save no longer wipes Settings-page preferences (#204, P2.12, data-integrity)**: frontend-only, PR #288. `ProfilePage.onSubmit` PATCHed `/users/me` with a 3-field `preferences` object; the backend `$set`-replaces the whole subdocument, so a bio-only save silently destroyed every Settings-page preference (demo pins it live: 11 stored fields → 3 on main). Fix per the CodeRabbit plan (design choice pre-resolved: merge + load guard): the page retains the full loaded `UserPreferences`, merges the 3 editable fields on save (the Settings-page contract), calls `invalidateUserPreferencesCache` post-save, and disables Save until preferences load (`'loading'|'loaded'|'error'`; error shows an alert — a failed load can't arm the wipe). **Two real pre-existing bugs fixed en route**: (1) the hydration dirty-guard read `form.formState.isDirty` in an async callback, but RHF's formState is a **lazy Proxy** — nothing read `isDirty` during render, so the guard was always false and a slow fetch clobbered in-progress edits (the legacy suite mocks RHF wholesale, so it never caught this); dirty tracking is now subscribed during render and hydration uses `reset(values, {keepDirtyValues: true})`; (2) codex pre-PR P2 — an all-or-nothing dirty skip would have left `bio`/`display_name` at session defaults (`''`) that a save writes over stored data; keepDirtyValues hydrates untouched fields while retaining edits. Post-PR fresh codex session P2 also fixed: a session **user switch** left `loadState='loaded'` with the prior account's retained preferences saveable into the new account — the effect now re-arms (`loading` + `{}`) on entry. Test-contract adaptation: `ProfilePage.test.tsx` mocks `useProfileApi` **without** `getUserProfile`, so the page deliberately treats an absent hook as loaded (test-only path; hook always present in production). New `ProfilePageSave.test.tsx` (real RHF + real `useProfileApi` over mocked `useAuthFetch`, the `SettingsPageSave` idiom; the legacy suite mocks RHF so it can't express the merge contract): 5 tests — merge preservation incl. a `future_flag`, cache invalidation, load-failure guard, dirty-path hydration, user-switch re-arm — **all mutation-verified RED** (merge-spread removal, dirty-subscription removal, keepDirtyValues removal, re-arm removal each fail exactly their tests). Reviews: opencode (GLM) occupied by a foreign-cwd delegation both rounds → **codex fallback pre-PR + post-PR fresh sessions** (the 2 P2s above), triage posted to the PR. Demo (`docs/demos/2026-07-13-issue-204-profile-save-preserves-prefs.md`): real backend + local Mongo + genuine better-auth signup, branch :3000 vs pristine main worktree :3001, same user — main's bio-only save wipes the subdocument to 3 fields with a success toast; branch preserves all 11 + the bio change (post-save Mongo docs shown); dead backend → alert + disabled Save. NB one-way state transitions → `showboat verify` intentionally diffs on mid-narrative blocks (#203 precedent). Frontend **117 suites, 2123 passed / 5 skipped**. Env note: the backend reads `MONGODB_URI`/`DATABASE_URL` (local Mongo here) — the `DATABASE_URI` Atlas line in `backend/.env` is not a var the config consumes.
  - **Status**: ✅ Complete
