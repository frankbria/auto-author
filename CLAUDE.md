...
- `backend/requirements.txt` is a generated export of `uv.lock`; **never edit it by hand** — regenerate it (`Security Audit` prints the command) after any `pyproject.toml`/`uv.lock` change. Dependabot's update scans no longer see it (#512 — alerts still do), so backend dep PRs need that regen on the branch. Procedure: `docs/references/quality-standards.md` → Backend Dependency Updates.
